...
function get_game_coin4(){
	var money = parseInt($('#money12').val());

	if(!money){
		money = parseInt($('#goldpig input[name="money"]:checked').val());
	}else{
		$('#goldpig input[name="money"]:checked').attr('checked', false).parent().removeClass('money_checked');
	}
	
	if(isNaN(money)) money = 0;
	$('#game_coin4').html(money * 1);
}
...
$(function(){
	 ...
	 * @author cb
	 */
	$("#goldpig_action").on("click", function(){
		
		if($.trim($("#toaccount12").val()).length == 0){
			alert("充值账号不能为空!");
			return false;
		}

		if($.trim($("#retoaccount12").val()).length != $.trim($("#toaccount12").val()).length){
			alert("两次账号不相同!");
			return false;
		}

		if($('#goldpig input[name="money"]:checked').val() == undefined){
			if($("#money12").val() == 0){
				alert("请输入充值金额");
				return false;
			}
		}
		var amount = $('#goldpig input[name="money"]:checked').val() == undefined?$("#money12").val():$('#goldpig input[name="money"]:checked').val();

		$("#goldpig_amount").val(amount);
		$("#form_goldpig").submit();
